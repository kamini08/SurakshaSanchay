...
const ResultDisplayQr: React.FC<ResultDisplayProps> = ({ result, onClear }) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await fetch("/api/adminPages/qrMaintenance", {
        method: "PUT",
  ...
  return (
    <div className="mx-auto mt-6 w-full max-w-md rounded-lg bg-white/20 p-6 text-black shadow-md outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary">
      ...
      {userRole === "user" ? (
        <div className="mb-4">
          <label className="mb-2 block text-xl text-white/70">
            Issue Discription
            <input
              type="text"
              value={temporaryLocation}
              onChange={(e) => setTemporaryLocation(e.target.value)}
              className="focus:ring-blue -500 mt-1 block w-full rounded-md border border-gray-300 p-2 text-black-2 focus:outline-none focus:ring"
...
export default ResultDisplayQr;
